...
const CategoryPage = () => {
  ...
  const handleAddCategory = async (addingCategory) => {
    try {
      setBtnLoading(true);
      await addCategory({ addingCategory: addingCategory });
  ...
  const handleUpdateCategory = async (cat) => {
    setBtnLoading(true);
    try {
      await updateCategory({ updatingCategory: cat });
          ...
          <div style={{ fontSize: 16, position: "relative" }}>
            ...
            >
              <Button
                ...
                className="btn-save"
                icon={<SaveOutlined />}
                onClick={async () => await handleAddCategory(addingCategory)}
                style={{
                  minWidth: 100,
